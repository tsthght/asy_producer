...
//export AsyncMessage
func AsyncMessage (db, tb, sql *C.char, cts, ats, tso, seq C.long) *C.char {
	if p == nil {
		return C.CString("")
	}

	m := message.Message{C.GoString(db), C.GoString(tb), C.GoString(sql), int64(cts), int64(ats), int64(tso), int64(seq)}
...
//export GetSafeMode
func GetSafeMode() C.int {
	return C.int(p.GetSafeMode())
}
